...
@Component({
    ...
    template: `<div [id]="card.id" class="flex bg-surface-0 dark:bg-surface-900 flex-col w-full border border-surface p-4 gap-8 hover:bg-surface-50 dark:hover:bg-surface-950 cursor-pointer rounded-border" cdkDragHandle>
        ...
        <div class="flex items-center justify-between flex-col md:flex-row gap-6 md:gap-0">
            <p-avatar-group>
                <p-avatar *ngFor="let assignee of card.assignees | slice: 0 : 3" image="/images/avatar/{{ assignee.image }}" shape="circle" styleClass="border-2 border-surface"></p-avatar>
            ...
            <div *ngIf="card.attachments || card.dueDate" class="flex items-center gap-4">
                <span class="text-surface-900 dark:text-surface-0 font-semibold" *ngIf="card.taskList.tasks.length"><i class="pi pi-check-square text-surface-700 dark:text-surface-100 mr-2"></i>{{ generateTaskInfo() }}</span>
                <span class="text-surface-900 dark:text-surface-0 font-semibold" *ngIf="card.attachments"><i class="pi pi-paperclip text-surface-700 dark:text-surface-100 mr-2"></i>{{ card.attachments }}</span>
                <span class="text-surface-900 dark:text-surface-0 font-semibold" *ngIf="card.dueDate"><i class="pi pi-clock text-surface-700 dark:text-surface-100 mr-2"></i>{{ parseDate(card.dueDate) }}</span>
            </div>
        </div>
    </div>`
})
